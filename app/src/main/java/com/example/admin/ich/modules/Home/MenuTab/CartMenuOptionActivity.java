...
public class CartMenuOptionActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    @NonNull
    ...
    private CartRadioAdapter radioAdapter;
    ...
    String checkboxId = "",menuoptionid,menuoptionidCheckbox,menuoptionidSelect,menuoptionNameSelect;
    String radioId = "-1";
    String selectId = "-1";

    IchDao dao;
    ArrayList<Menu> product;
    ArrayList<String> menusList, menuTotalList, menuCountList,menuCartRadioIdList,menuCartCheckIdList,menuCartSelectIdList,menuCartSelectNameList;
    int totalCartCount;
    double total_price;
    ArrayList<OptionJson> cartRadioarray,cartCheckboxarray,cartSelectarray;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        menuCartRadioIdList = new ArrayList<>();
        menuCartCheckIdList = new ArrayList<>();
        menuCartSelectIdList = new ArrayList<>();
        menuCartSelectNameList = new ArrayList<>();
        ...
        cartRadioarray = dao.getCartRadio(menuId);
        for (OptionJson i : cartRadioarray) {
            menuCartRadioIdList.add(i.getMenu_option_value_id());
        }
        menuoptionid=menuCartRadioIdList.toString();
        menuoptionid=menuoptionid.substring(1,menuoptionid.length()-1);

        cartCheckboxarray = dao.getCartCheckbox(menuId);
        for (OptionJson i : cartCheckboxarray) {
            menuCartCheckIdList.add(i.getMenu_option_value_id());
        }
        menuoptionidCheckbox=menuCartCheckIdList.toString();
        menuoptionidCheckbox=menuoptionidCheckbox.substring(1,menuoptionidCheckbox.length()-1);

        cartSelectarray = dao.getCartSelect(menuId);
        for (OptionJson i : cartSelectarray) {
            menuCartSelectIdList.add(i.getMenu_option_value_id());
            menuCartSelectNameList.add(i.getValue()+"  "+i.getOption_price());
        }
        menuoptionidSelect=menuCartSelectIdList.toString();
        menuoptionNameSelect=menuCartSelectNameList.toString();
        menuoptionidSelect=menuoptionidSelect.substring(1,menuoptionidSelect.length()-1);
        menuoptionNameSelect=menuoptionNameSelect.substring(1,menuoptionNameSelect.length()-1);


       // Toast.makeText(CartMenuOptionActivity.this, menuoptionid, Toast.LENGTH_SHORT).show();
        getmenus(menuId);
    ...
    public void getmenus
        ...
        final ProgressDialog progressDialog = new ProgressDialog(CartMenuOptionActivity.this);
                ...
                if (response.code() == 200) {
                    if (response.isSuccessful()) {
                        if (menuoptions.contains("radio")) {
                            ...
                            mAdapter = new CartCheckboxAdapter(menuListRadio,menuoptionid);
                            ...
                            radioAdapter = new CartRadioAdapter(getApplicationContext(), menuListRadio,menuoptionid);
                            radioRecycle.setAdapter(radioAdapter);
                            radioRecycle.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                            radioAdapter.setOnItemClickListener(CartMenuOptionActivity.this);
                        ...
                        if (menuoptions.contains("checkbox")) {
                            ...
                            checkboxRecycle.setHasFixedSize(true);
                            checkboxRecycle.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
                            mAdapter = new CartCheckboxAdapter(menuListCheckbox,menuoptionidCheckbox);
                            checkboxRecycle.setAdapter(mAdapter);
                        ...
                        if (menuoptions.contains("select")) {
                            ...
                            dropdownList.add(0, "Select Options");
                            for (int i = 0; i < menuListSelect.size(); i++) {

                                dropdownList.add(menuListSelect.get(i).getValue() + menuListSelect.get(i).getPrice());
                            }
                            // Creating adapter for spinner service
                            final ArrayAdapter<String> mAdapter = new ArrayAdapter<String>(CartMenuOptionActivity.this, android.R.layout.simple_spinner_item, dropdownList);
                            mAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);

                            // attaching data adapter to spinner;
                            spinnerSelect.setAdapter(mAdapter);

                          //  spinnerSelect.setSelection(((ArrayAdapter<String>) spinnerSelect.getAdapter()).getPosition(menuoptionidSelect));
                            if (!menuoptionidSelect.equals(null)) {
                                int spinnerPosition = mAdapter.getPosition(menuoptionidSelect);
                                spinnerSelect.setSelection(spinnerPosition);

                            }
                            input_select.setText(menuoptionNameSelect );
                            ...
                            spinnerSelect.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                                ...
                                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                                    ...
                                    if (position != 0) {
                                        ...
                                        double total_price = (Integer.parseInt(count) * Double.parseDouble(menuListSelect.get(position-1).getPrice().trim()));
                                    ...
                                    else
                                    {
                                      //  input_select.setText(spinnerSelect.getSelectedItem().toString() );
                                        ...
                                        optionJson.setOption_name(option_name_select);
                                        optionJson.setDisplay_type(display_type_select);
                                        optionJson.setOption_value_id(menuListSelect.get(position).getOptionValueId());
                                        optionJson.setMenu_option_value_id(menuListSelect.get(position).getMenuOptionValueId());
                                        optionJson.setValue(menuListSelect.get(position).getValue());
                                        optionJson.setOption_price(menuListSelect.get(position).getPrice());
                                        double total_price = (Integer.parseInt(count) * Double.parseDouble(menuListSelect.get(position).getPrice().trim()));
                                        optionJson.setOption_total(String.valueOf(total_price));
                                        menuListOptionSelect.add(optionJson);
                                 }
    ...
    public void AddCart() {
       ...
       if (menuoptions.contains("checkbox")) {
            List<OptionValue> stList = ((CartCheckboxAdapter) mAdapter).getOptionList();
            ...
            Toast.makeText(CartMenuOptionActivity.this, myCustomArray.toString(), Toast.LENGTH_SHORT).show();
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
      ...
      //  radioId = menuListRadio.get(position).getOptionValueId();
       // Toast.makeText(CartMenuOptionActivity.this, String.valueOf(position), Toast.LENGTH_SHORT).show();
        OptionJson optionJson = new OptionJson();
        optionJson.setCount(count);
        optionJson.setMenu_id(menuId);
